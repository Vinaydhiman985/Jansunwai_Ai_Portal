...
def get_dashboard_statistics(department: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    status_counts = {}
    for s in statuses:
        s_query = db.query(Complaint).filter(Complaint.status == s)
        if department:
            s_query = s_query.filter(Complaint.department == department)
        status_counts[s] = s_query.count()
...
def get_complaint_trends(days: int = 14, db: Session = Depends(get_db)):
    ...
    filed_rows = (
        ...
        .filter(func.date(Complaint.created_at) >= start_date)
    ...
    resolved_rows = (
        db.query(
            func.date(Complaint.updated_at).label("day"),
            func.count(Complaint.id).label("count")
        )
        .filter(Complaint.status == "Resolved")
        .filter(func.date(Complaint.updated_at) >= start_date)
        .group_by(func.date(Complaint.updated_at))
        .all()
    )

    filed_map = {str(row.day): row.count for row in filed_rows}
